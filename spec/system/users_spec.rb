...
RSpec.describe "Users", type: :system do
  ...
  describe "新規登録する" do
    ...
    context "有効な値を入力する場合" do
      ...
      context "メール認証前" do
        ...
        it "登録したメールアドレスで、別のアカウントを作成しようとするとエラーが表示される" do
          click_button '新規登録'
          click_link '新規登録'
          fill_in 'user_email', with: 'takayoshi@example.com'
          click_button '新規登録'
      ...
      context "メール認証後" do
        ...
        it "メール認証ができたメッセージが表示される" do
          expect(page).to have_content 'メールアドレスが確認できました。' 
          expect(current_path).to eq new_user_session_path
        end
        it "登録した情報が、ユーザ一覧ページに表示される" do
          click_link 'ユーザ一覧'
          expect(page).to have_content 'たかよし'
        end
        it "登録したユーザでログイン、ログアウトができる" do
          fill_in 'user_email', with: 'takayoshi@example.com'
          fill_in 'user_password', with: 'password'
          click_button 'ログイン'
        ...
        context "ログイン後" do
          ...
          it "登録した情報が、ユーザ情報ページに表示される" do
            click_link 'マイページ'
            expect(page).to have_content 'たかよし'
          end
          it "登録した情報が、ユーザ編集ページに表示される" do
            click_link 'マイページ'
            click_link '編集'
    ...
    context "無効な値を入力する場合" do
      ...
      it "ニックネームが51文字の場合、エラーが表示される" do
        char51 = 'a' * 51
      ...
      it "パスワードが5文字以下の場合、エラーが表示される" do
  ...
  describe "ログインする" do
    ...
    context "無効な値を入力する場合" do
      ...
      it "メールアドレスは正しいが、パスワードが正しくない場合、エラーが表示される" do
        fill_in 'user_email', with: 'kenij@example.com'
        fill_in 'user_password', with: 'abcdefgh'
        click_button 'ログイン'
        expect(page).to have_content 'メールアドレスまたはパスワードが違います。'
      end
      it "メールアドレスは正しくないが、パスワードが正しい場合、エラーが表示される" do
  ...
  describe "ログインし、ユーザ情報を変更する" do
    ...
    context "有効な値を入力する場合" do
      ...
      context "ユーザイメージ、ニックネーム、パスワードを変更するを変更する場合" do
        ...
        it "変更ができたメッセージが表示される"  do
          expect(page).to have_content 'アカウント情報を変更しました。'
          expect(current_path).to eq root_path
        end
        it "変更した情報が、ユーザ一覧ページに表示される"  do
          click_link 'ユーザ一覧'
          expect(page).to have_content 'ミナカワけんじ'
        end
        it "変更した情報が、ユーザ情報ページに表示される"  do
          click_link 'マイページ'
          expect(page).to have_content 'ミナカワけんじ'
        end
        it "変更した情報が、ユーザ編集ページに表示される"  do
          click_link 'マイページ'
          click_link '編集'
          expect(page).to have_xpath"//input[@id='user_nickname'][@value='ミナカワけんじ']"
        end
        it "変更した情報でログイン、ログアウトができる"  do
          click_link 'ログアウト'
          expect(page).to have_content 'ログアウトしました。' 
          click_link 'ログイン'
          fill_in 'user_email', with: 'kenji@example.com'
      ...
      context "メールアドレスを変更する場合" do
        ...
        it "メールが送信され、その旨のメッセージが表示される"  do
        ...
        context "メール認証後" do
          ...
          it "メールアドレスを確認できたメッセージが表示される"  do
            expect(page).to have_content 'メールアドレスが確認できました。'
            expect(current_path).to eq root_path
          end
          it "変更した情報が、ユーザ編集ページに表示される"  do
    ...
    #  -----  無効な値  -----  #
    context "無効な値を入力する場合" do
      it "現在のパスワード空欄の場合、エラーが表示される" do
        fill_in 'user_current_password', with: ''
        click_button '変更する'
        expect(page).to have_content 'ユーザ は保存されませんでした。'
        expect(page).to have_content '現在のパスワードを入力してください'
      end
      it "現在のパスワードが正しくない場合、エラーが表示される" do
      ...
      it "ニックネームの値が51文字の場合、エラーが表示される" do
        char51 = 'a' * 51
        fill_in 'user_nickname', with: char51
        fill_in 'user_current_password', with: 'password'
        click_button'変更する'
      ...
      it "新しいパスワードが5文字以下の場合、エラーが表示される" do
        fill_in 'user_nickname', with: 'new_kenji'
  ...
  describe "ユーザはログインしなければ、ユーザ情報ページをできない" do
    it "ユーザ情報ページが表示できず、エラーメッセージが表示される" do
      visit users_path
      click_link "けんじ"
  ...
  describe "ユーザのパスワードを再設定する" do
    ...
    context "存在するメールアドレスを送信する場合" do
      ...
      it "再設定についてのメールを送信する旨のメッセージが表示される" do
